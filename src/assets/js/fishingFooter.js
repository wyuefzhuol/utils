export default {
  ...
  init: function(containerId) {
    this.setParameters(containerId);
    this.reconstructMethods();
    this.setup();
    this.bindEvent();
    this.render();
  },
  setParameters: function(containerId) {
  ...
  getAxis: function(event) {
    ...
    const offsetTop = box.top + window.pageYOffset - document.documentElement.clientTop;
    const offsetLeft = box.left + window.pageXOffset - document.documentElement.clientLeft;

    return {
      x: event.clientX - offsetLeft + window.scrollLeft,
      y: event.clientY - offsetTop + window.scrollTop
    };
  },
  ...
  reverseVertical: function() {
    ...
    let i = 0, count = this.fishes.length;
    for (; i < count; i++) {
